        ...
        if form.is_valid():
            ...
            if request.user.is_authenticated:
                ...
                try:
                    ...
                    admin_email_sent = send_website_request_email(
                    ...
                    if admin_email_sent:
                        logger.info(f"Admin notification sent for request ID {website_request.id}")
                except Exception as e:
                    logger.error(f"Error sending admin email for request ID {website_request.id}: {type(e).__name__}: {str(e)}")
                
                # Send confirmation email to client
                try:
                    client_email_sent = send_website_request_confirmation(website_request.email, website_request.business_name)
                    if client_email_sent:
                        logger.info(f"Confirmation email sent to {website_request.email}")
                except Exception as e:
                    logger.error(f"Error sending confirmation email to {website_request.email}: {type(e).__name__}: {str(e)}")
        ...
        if user is not None:
            ...
            if 'website_request_data' in request.session:
                ...
                try:
                    ...
                    logger.info(f"Admin notification sent for request ID {website_request.id} (after login)")
                except Exception as e:
                    logger.error(f"Error sending admin email after login for request ID {website_request.id}: {type(e).__name__}: {str(e)}")
                
                # Send confirmation email to client
                try:
                    send_website_request_confirmation(website_request.email, website_request.business_name)
                    logger.info(f"Confirmation email sent to {website_request.email} (after login)")
                except Exception as e:
                    logger.error(f"Error sending confirmation email after login to {website_request.email}: {type(e).__name__}: {str(e)}")
    ...
    if request.method == 'POST':
        ...
        if 'website_request_data' in request.session:
            ...
            try:
                ...
                logger.info(f"Admin notification sent for request ID {website_request.id} (after signup)")
            except Exception as e:
                logger.error(f"Error sending admin email after signup for request ID {website_request.id}: {type(e).__name__}: {str(e)}")
            
            # Send confirmation email to client
            try:
                send_website_request_confirmation(website_request.email, website_request.business_name)
                logger.info(f"Confirmation email sent to {website_request.email} (after signup)")
            except Exception as e:
                logger.error(f"Error sending confirmation email after signup to {website_request.email}: {type(e).__name__}: {str(e)}")
